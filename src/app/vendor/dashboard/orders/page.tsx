              ...
              <tbody className="divide-y divide-gray-50">
                {items.map((item: import('@/types').OrderItem & { order: { orderNumber: string; createdAt: string } }) => (
                  <tr key={item.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-5 py-4 font-bold text-sm">₦{Number(item.totalPrice).toLocaleString()}</td>
                    <td className="px-5 py-4 font-bold text-sm text-green-600">₦{Number(item.vendorEarning).toLocaleString()}</td>
                    <td className="px-5 py-4">

      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
        <div className="space-y-4">
          ...
          <button onClick={track} disabled={loading}
            className="w-full py-3 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm disabled:opacity-60">
      ...
      {order && (
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-xs font-bold px-3 py-1.5 rounded-full ${STATUS_CLASS[order.status] || 'bg-gray-100 text-gray-700'}`}>{order.status}</span>
          </div>
                ...
                {STATUS_STEPS.map((step, i) => (
                  <div key={step} className="flex-1 flex items-center">
                    <div className="flex flex-col items-center">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i <= stepIdx ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-400'}`}>
                        {i < stepIdx ? '✓' : i === stepIdx ? '●' : '○'}
                      </div>
                      <span className="text-[9px] text-gray-400 mt-1 capitalize text-center max-w-[50px] leading-tight">{step.replace('_',' ')}</span>
                    </div>
                    {i < STATUS_STEPS.length - 1 && <div className={`flex-1 h-0.5 mx-1 mb-4 ${i < stepIdx ? 'bg-orange-500' : 'bg-gray-200'}`} />}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-2 mb-4">
            {order.items?.map((item: any) => (
              <div key={item.id} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center text-base shrink-0">
                  {item.productImage ? <img src={item.productImage} alt="" className="w-full h-full object-cover rounded-lg" /> : '📦'}
                </div>

            ...
            <div key={p.id} className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                <div className="bg-gray-50 rounded-xl p-3 text-center">
                  <p className="text-lg font-extrabold text-gray-800">₦{Number(p.amount).toLocaleString()}</p>
                ...
                <div className="bg-gray-50 rounded-xl p-3 text-center">
                  ...
                  <p className="text-xs font-extrabold text-green-600">₦{Number(p.vendor?.availableBalance || 0).toLocaleString()}</p>
              ...
              {p.status === 'pending' && (
                <div className="flex gap-2">
                  <button onClick={() => doAction(p.id, 'approve', p.vendor?.businessName, Number(p.amount).toLocaleString())} className="flex-1 py-2.5 rounded-xl text-sm font-bold bg-green-100 text-green-700 hover:bg-green-600 hover:text-white transition-colors">✅ Approve</button>
                  <button onClick={() => doAction(p.id, 'mark_paid', p.vendor?.businessName, Number(p.amount).toLocaleString())} className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white transition-colors" style={{ background: '#1e8a44' }}>💸 Approve & Mark Paid</button>
              ...
              {p.status === 'approved' && (
                <button onClick={() => doAction(p.id, 'mark_paid', p.vendor?.businessName, Number(p.amount).toLocaleString())} className="w-full py-2.5 rounded-xl text-white text-sm font-bold" style={{ background: '#1e8a44' }}>💸 Mark as Paid</button>
              )}
            </div>
          )
        })}
